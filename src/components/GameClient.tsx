...
import { waitForConsensusReceipt } from '../services/genlayerTransactions';
import { TransactionStatus } from 'genlayer-js/types';

export default function GameClient({ lobby, onExit, difficulty = 'Medium', bossMode }: GameClientProps) {
  ...
  const handleSaveScore = async () => {
    if (gameOver === false || (finalKills === null && !bossMode) || !contractAddress || !client) {
      if (!client) {
        setSaveStatus("Please connect your wallet to save the score.");
    ...
    setIsSaving(true);
    setTxHash(null);
    setSaveStatus('Protocol Validators are evaluating performance & reaching consensus...');
    
    try {
      if (!strategy.trim()) {
        toast.error("Please enter your Cyberpunk Strategy!");
        setIsSaving(false);
        return;
      }
      ...
      let hash;
      if (bossMode) {
        const fullStrategy = `${strategy} (Context: I dealt ${Math.floor(damageDealt)} base damage in combat)`;
        hash = await genLayerService.attackWorldBoss(contractAddress, bossMode.boss_id, "Cyan", fullStrategy);
        setTxHash(hash as string);
        setSaveStatus('Consensus Reached: Attack Recorded. Check Boss details for the narrative outcome!');
        toast.success("Attack successful!");
      } else {
        hash = await genLayerService.submitMatch(contractAddress, difficulty, strategy, finalKills!, gameOver === 'won' ? 'VICTORY' : 'DEFEAT');
        setTxHash(hash as string);
        setSaveStatus('Consensus Reached: Match verified and recorded immutably.');
      ...
      setSaveStatus('Waiting for FINALIZED consensus receipt...');
      await waitForConsensusReceipt(hash, TransactionStatus.FINALIZED);

      setSaveStatus('Validated and saved on-chain successfully!');
      toast.success("Match verified by Validators and saved on-chain!");
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    const botPlayers = (() => {
      if (bossMode) {
        const pos = getValidSpawnPosition(botEntities, w, h, minPadding, 100);
        botEntities.push({ x: pos.x, y: pos.y, radius: 100 });
        return [{
            x: pos.x, y: pos.y, vx: 0, vy: 0, radius: 50,
            color: getRandomColorObj(), trail: [] as {x: number, y: number, life: number}[],
            targetX: w/2, targetY: h/2, waitTimer: 0, hp: 500, isBoosting: false, isDead: false, respawnTimer: 0,
          name: `[BOSS] ${bossMode.boss_name}`,
          isBoss: true
        }];
      }
    ...
    const render = () => {
            ...
            if (dist < e1.radius + e2.radius) {
              ...
              const c2 = e2.color.fill;
               ...
               if (beats(c1, c2, e1.hp, e2.hp)) {
                 ...
                 if (e2.hp <= 25) {
                   e1.hp += e2.hp;
                   if (e1 === state.player && (e2 as any).isBoss) state.damageDealt += e2.hp;
                   e2.hp = 0;
                   ...
                   const dmg = 20;
                   if (e1 === state.player && (e2 as any).isBoss) state.damageDealt += dmg;
                   e2.hp -= dmg;
                   e1.hp += 10;
                   e1.vx -= nx * 4.5;
                   e1.vy -= ny * 4.5;
                   e2.vx += nx * 9;
                   e2.vy += ny * 9;
                   addFloatingText(e2.x, e2.y, `-${dmg}`, "#ef4444");
      ...
      if (state.player.isDead) {
         ...
         setDamageDealt(state.damageDealt);
         setGameOver('lost');
         audio.playLose();
  ...
  return (
    <div className="relative w-full h-full flex flex-col flex-1 overflow-hidden bg-gen-bg">
      {/* Game Field Container */}
      <div className="flex-1 w-full relative min-h-0 border-b-2 border-white/10">
        <canvas 
          ref={canvasRef}
        ...
        {gameOver && (
          <div className="absolute inset-0 z-30 bg-[#050505]/80 backdrop-blur-sm pointer-events-auto flex items-center justify-center p-4">
            <div className="flex flex-col items-center w-72 p-4 bg-[#0a0a0a]/95 border border-white/10 shadow-2xl gap-3">
            
            {/* Title */}
            {gameOver === 'won' ? (
               <div className="text-center">
                 <h2 className="font-bebas text-3xl text-gen-lime tracking-widest leading-none">VICTORY</h2>
                 <p className="font-mono text-[10px] text-white/70 tracking-widest uppercase mt-0.5">The algorithm bows to you.</p>
               </div>
            ) : (
               <div className="text-center">
                 <h2 className="font-bebas text-3xl text-red-500 tracking-widest leading-none">DESTROYED</h2>
                 <p className="font-mono text-[10px] text-gen-text-muted tracking-widest uppercase mt-0.5">Consensus rejected.</p>
               </div>
            )}

            {/* Score */}
            <div className="text-center bg-black/40 border border-white/5 px-3 py-2 w-full">
              <span className="text-[9px] font-mono text-gen-text-muted tracking-widest uppercase block">MATCH PERFORMANCE</span>
              <div className="text-2xl font-bebas text-white tracking-widest leading-none mt-0.5">
                {bossMode ? Math.floor(damageDealt) : finalKills} <span className="text-sm text-gen-lime">{bossMode ? 'DAMAGE' : 'KILLS'}</span>
              </div>
            </div>

            {/* Protocol Sync */}
            <div className="w-full border border-white/5 bg-black/40 p-3 text-center flex flex-col items-center gap-2">
              <span className="text-[9px] font-mono tracking-widest text-gen-text-muted uppercase">GENLAYER PROTOCOL SYNC</span>
              ...
              {!txHash && !isSaving && (
                 <div className="flex flex-col w-full gap-2">
                   <textarea
                     ...
                     placeholder={bossMode ? "How did you attack the boss? (E.g. I used an EMP blast)" : "Describe your strategy..."}
                     className="w-full bg-black/80 border border-white/10 p-1.5 text-white font-mono text-[10px] h-12 resize-none focus:outline-none focus:border-gen-lime transition-colors"
                   />
                   <button
                     onClick={handleSaveScore}
                     className="px-3 py-1.5 bg-gen-lime hover:bg-white text-gen-bg font-bebas text-sm tracking-widest transition-colors w-full leading-none"
                   >
                     {bossMode ? 'SUBMIT COMBAT LOG' : 'SUBMIT TO GAME MASTER'}
                   </button>
                 </div>
              )}

              <div className="flex items-center gap-1.5 text-[10px] font-mono text-center">
                {isSaving ? (
                  <Loader2 className="w-3 h-3 text-gen-lime animate-spin flex-shrink-0" />
                ) : txHash ? (
                  <ShieldCheck className="w-3 h-3 text-gen-lime flex-shrink-0" />
                ) : null}
                <span className={txHash ? "text-gen-lime font-bold tracking-widest" : "text-gen-text"}>
                  {saveStatus || 'AWAITING MANUAL SAVE...'}
                </span>
              </div>

              {txHash && (
                <div className="w-full text-center mt-1 border-t border-white/10 pt-1.5">
                  <span className="text-[8px] font-mono tracking-widest text-gen-text-muted block mb-0.5">TX HASH</span>
                  <span className="font-mono text-[9px] text-gen-lime bg-black/50 py-0.5 px-1.5 block w-full truncate select-all">
                    {txHash}
                  </span>
                </div>
              )}
            ...
            <button 
              onClick={onExit}
              disabled={isSaving}
              className={`px-3 py-1.5 font-bebas text-sm tracking-widest transition-colors flex items-center justify-center gap-1.5 w-full border leading-none ${isSaving ? 'bg-gen-surface text-gen-text-muted border-white/10 cursor-not-allowed' : 'bg-transparent text-white border-white hover:bg-white hover:text-gen-bg'}`}
            >
              {isSaving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Zap className="w-3 h-3" />}
              <span>{isSaving ? 'SYNCHRONIZING...' : 'RETURN TO LOBBIES'}</span>
            </button>
            </div>
          </div>
        )}
      ...
      {/* Brutalist HUD */}
      <div className="h-[100px] bg-gen-bg flex items-center justify-between px-4 md:px-10 relative z-20 select-none">
        {/* Left panel info */}
        <div className="flex items-center gap-8">
           <button 
             onClick={onExit}
             className="bg-transparent hover:bg-red-500 hover:text-gen-bg text-red-500 border-2 border-red-500/50 px-6 py-3 font-bebas text-2xl tracking-widest transition-colors pointer-events-auto cursor-pointer"
           >
             ABORT
           </button>
           
           <div className="hidden md:flex items-center gap-8 font-mono text-gen-text-muted tracking-widest uppercase text-sm">
              <div className="flex items-center gap-3">
                KILLS: <span id="hud-match-kills" className="text-white font-bold text-2xl">0</span>
              </div>
              <div className="w-px h-8 bg-white/10" />
              <div className="flex items-center gap-3">
                REMAINING: <span id="hud-bots-count" className="text-red-500 font-bold text-2xl">--</span>
              </div>
           </div>
        </div>

        {/* Right panel HP info */}
        <div className="flex items-center gap-6 bg-gen-surface border-2 border-white/10 px-6 py-2">
           <span id="hud-player-hp" className="text-white text-4xl md:text-5xl font-bebas tracking-widest">100 HP</span>
           <div id="hud-hp-chip" className="w-12 h-12 flex items-center justify-center relative border-4 border-white transition-all bg-gen-bg">
