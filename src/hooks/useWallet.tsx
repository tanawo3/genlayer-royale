...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const deployContract = useCallback(async () => {
    ...
    setIsDeploying(true);
    try {
      // Always use a fresh local account for deployment to avoid MetaMask signature prompts
      const deployClient = createClient({
        chain: studionet,
        account: createAccount(),
      });

      const hash = await deployClient.deployContract({
          code: GAME_CONTRACT_CODE,
          args: [],
      });
      const types = await import('genlayer-js/types');
      try {
         const receipt = await deployClient.waitForTransactionReceipt({
             hash: hash as any,
          ...
          for (let i = 0; i < 12; i++) {
            try {
              await deployClient.readContract({
